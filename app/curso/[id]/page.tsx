...
export default function CursoPage() {
    ...
    <h1 className="text-6xl font-black text-white mb-10 tracking-tighter animate-in fade-in slide-in-from-top-6 duration-1000">
        Detalhes do <span className="bg-linear-to-r from-orange-400 via-rose-500 to-fuchsia-600 bg-clip-text text-transparent">Curso</span>
        <Link href={`/curso/${id}/editar`} className="text-zinc-300 hover:text-violet-400 font-medium transition-all duration-700 ease-in-out hover:scale-105 active:scale-95">
        <PenBox />
        </Link>
    </h1>
    ...
    <div className="relative group w-full max-w-md">
        ...
        <div className="relative bg-zinc-900/80 backdrop-blur-2xl p-10 rounded-[2.5rem] border border-white/10 shadow-2xl transition-all duration-500 hover:border-orange-500/30 animate-in fade-in zoom-in-95">
            ...
            <div className="space-y-6 text-left">
                {[
                    { label: "ID", value: curso.id },
                    { label: "Nome", value: curso.nome },
                    { label: "Professor", value: curso.professor },
                    { label: "Carga Horária", value: curso.cargaHoraria },
                    { label: "Descrição", value: curso.descricao }
                ].map((item, i) => (
                    <div 
                        key={item.label}
